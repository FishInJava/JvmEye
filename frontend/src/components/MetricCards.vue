  ...
  <div class="cards">
    <article class="card">
      <header class="card-head">
        ...
        <span v-if="heapPercent !== null" class="badge" :class="heapPercent > HEAP_WARN_PERCENT ? 'badge-danger' : 'badge-ok'">
    ...
    <article class="card">
      ...
      <div class="chips">
        <span v-for="(count, state) in threadStates" :key="state" class="chip" :class="stateChipClass(state, count)">
          {{ state }} {{ count }}
        </span>
      </div>
    </article>
    ...
    <article class="card">
      ...
      <div class="card-sub dim">
        系统 {{ formatPercent(cpu?.systemCpuLoad) }} · LoadAverage {{ formatDecimal(cpu?.systemLoadAverage) }} · {{ cpu?.availableProcessors ?? '-' }} 核
      </div>
      <dl class="card-extra">
...
import { formatBytes, formatDecimal, formatDuration, formatNumber, formatPercent } from '../utils/format'
...
/** 堆占用百分比;heapMax <= 0(未设置上限)时无法计算,返回 null 不显示徽标。 */
...
/** 线程状态 → chip 配色:RUNNABLE 绿、BLOCKED 且确实有线程时红、其余弱化。 */
function stateChipClass(state, count) {
